...
const PdfContent = ({ file }) => {
  ...
  useEffect(() => {
    ...
    if(typeof window !== 'undefined') {  // Check if window is defined (this means we're on the client-side)
      ...
      return () => mediaQuery.removeEventListener('change', setDarkModeFromMediaQuery); // Cleanup function to remove the event listener
    }
  }, []);
  ...
  return (
    <div className="pdf-viewer">
      <div className="pdf-container">
        <div className="pdf-navigation previous" onClick={() => setPageNumber(Math.max(pageNumber - 1, 1))} >
          <FaArrowCircleLeft />
        </div>
        <Document
          file={file}
        ...
        <div className="pdf-navigation next" onClick={() => setPageNumber(Math.min(pageNumber + 1, numPages))}>
          <FaArrowCircleRight />
        </div>
