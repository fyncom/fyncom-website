...
# Configuration
SITE_URL = 'sc-domain:fyncom.com'
THRESHOLD_PERCENT = int(os.environ.get('THRESHOLD_PERCENT', 10))
ALERT_EMAIL = 'support@fyncom.com'
SMTP_EMAIL = 'support@fyncom.com'
...
"""
            send_alert(
                f"🚨 CRITICAL: FynCom has {current_count} indexed pages (DAILY ALERT)",
                message
            )
...
"""
                    send_alert(
                        f"⚠️ FynCom: {abs(change_percent):.0f}% Indexing Drop Detected",
                        message
                    )
...
"""
        send_alert("❌ FynCom: Indexing Monitor Error", error_message)
        print(f"\n❌ error: {str(e)}")
        sys.exit(1)
